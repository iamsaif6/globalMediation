...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="fixed z-50 md:max-w-[1380px] border border-primary left-1/2 -translate-x-1/2 w-full mx-auto flex items-center justify-between bg-white max-w-[90%] top-5 rounded-4xl py-[7px] md:py-3 md:pl-9 px-[18px] md:px-3">
          ...
          <ul className="md:flex relative hidden gap-6 text-secondary font-normal text-lg items-center">
            {navLinks.map(link => {
              return (
                <li
                  key={link.href}
                  onMouseEnter={() => link.hasMegaMenu && setActiveMenu(link.title)}
                  onMouseLeave={() => setActiveMenu(null)}
                >
                  <Link href={link.href} className={`px-2 py-4 flex items-center ${link.hasMegaMenu ? 'gap-1' : ''}`}>
                    {link.title}
                    {link.hasMegaMenu && (
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className={`h-4 w-4 transition-transform duration-300 ${activeMenu === link.title ? 'rotate-180' : ''}`}
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                  ...
                  {link.hasMegaMenu && activeMenu === link.title && (
                    <div className="absolute top-full bg-white left-0 w-full bg-lavender-50 rounded-lg shadow-lg mt-1 transition-all duration-300 opacity-100 transform translate-y-0 z-50">
                      <div className="p-6 grid grid-cols-12 gap-6">
                        <div className="col-span-4 bg-lavender-100 p-6 rounded-lg">
                          <h3 className="text-xl font-semibold mb-2">{link.title === 'Services' ? 'Service' : link.title}</h3>
                          <p className="text-sm text-gray-600">A Comprehensive Guide to Alternative Dispute Resolution</p>
                        </div>
                        <div className="col-span-8 p-6">
                          <h3 className="text-xl font-semibold mb-4">
                            {link.title === 'Services' ? 'Mediation Services' : 'Mediation Training'}
                          </h3>
                          <ul className="space-y-3">
                            {link.megaMenuItems.map((item, index) => (
                              <li key={index} className="text-gray-600 hover:text-primary transition-colors">
      ...
      {/* Mobile Menu Overlay */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 bg-white z-50 overflow-y-auto md:hidden flex flex-col">
          ...
          <div className="flex-1 flex flex-col p-5 space-y-2">
            <Link href="/" className="py-4 px-4 text-lg rounded-lg bg-lavender-50 hover:bg-lavender-100">
              Home
            </Link>
                  ...
                  {link.hasMegaMenu ? (
                    <>
                      <button
                        ...
                        className="py-4 px-4 text-lg font-normal flex justify-between items-center bg-lavender-50 hover:bg-lavender-100 rounded-lg"
                      >
                        {link.title}
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                      ...
                      {mobileSubmenuOpen === link.title && (
                        <div className="mt-2 pl-4">
                          {link.megaMenuItems.map((item, index) => (
                            <Link
                              key={index}
                              href={item.href}
                              className="block py-3 px-4 text-gray-600 hover:text-primary border-b border-gray-100"
                            >
                              {item.title}
                            </Link>
                          ))}
                    ...
                    <Link href={link.href} className="py-4 px-4 text-lg font-normal bg-lavender-50 hover:bg-lavender-100 rounded-lg">
                      {link.title}
                    </Link>
                  )}
                </div>
