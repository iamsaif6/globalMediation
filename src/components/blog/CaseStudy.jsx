...
const CaseStudy = () => {
  ...
  const settings = {
    ...
    slidesToShow: 1,
    slidesToScroll: 1,
    customPaging: i => <div className="hidden md:block w-[50px] absolute top-[42px] mb-[72px]  h-1 rounded-xl bg-[#5C227E]"></div>,
  };
  ...
  return (
    <section className="py-16 px-5 md:py-[80px] md:px-[64px] bg-secondary text-white">
      <div className="flex gap-8 flex-col md:flex-row items-start md:items-end justify-between">
        ...
        <div className="flex justify-end gap-[15px] relative z-10 mt-4">
          <button
            type="button"
            aria-label="Previous slide"
            onClick={goPrev}
            className="flex justify-center items-center border border-[#fff] text-[#fff] rounded-full hover:bg-primary hover:border-primary hover:text-white cursor-pointer w-[35px] h-[35px] md:w-12 md:h-12"
          >
            <ArrowLeft />
          </button>
          <button
      ...
      <div className="slider-container slick-bar case py-[72px]">
          ...
          {[1, 2].map(item => (
            <div className=" rounded-xl overflow-hidden !grid grid-cols-12" key={item}>
              ...
              <div className="col-span-12  bg-white lg:col-span-7 py-[42px] md:py-[66px] px-6 md:pr-[74px] md:pl-[90px]">
                ...
                <Link
                  href={'#'}
                  className="flex w-max pb-1 border-b border-secondary  gap-1 items-center text-secondary font-semibold text-sm"
                >
                  <span>Read more</span>
